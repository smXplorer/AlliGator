# FLI_Dataset_to_Parameter_Map_Plugin_with_ROI_Mask_Example.py
...
def Very_Simple_Average_Lifetime_Map(
    ...
    ### Decay Fit Parameter Map:Decay Fit Parameter Map Image
    ### End of AlliGator Output Value Type & Destination ###

    # decode the dataset
    # note that some of the data element might be empty if none has been
    # defined in AlliGator (e.g. Reference_Decay or Mask_Image)
    ...
    mask = np.asarray(fli_dataset_data_in.Mask_Image)
    ...
    for i in range(size_x):
        for j in range (size_y):
            # only includes pixels that correspond to valid Mask ROIs
            if mask[j, i] != 0:
                irf_locations.append(alligator.location(X = i, Y = j))
                param_map.append([i, j, sum[j, i],mean_t[j, i]])
            else:
                param_map.append([i, j, float('nan'), float('nan')])
